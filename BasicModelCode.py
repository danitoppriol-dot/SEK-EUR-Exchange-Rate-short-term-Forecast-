...
# ============================================================================
# 1. DATA ACQUISITION - Feature Macro Fondamentali
# ============================================================================
...
# Crea DataFrame allineando tutti gli indici
print(f"\n✅ EUR/SEK data loaded: {len(eur_sek)} observations")

# Allinea tutti i dati all'indice di EUR/SEK
data = pd.DataFrame(index=eur_sek.index)
data['eur_sek'] = eur_sek

# Aggiungi altre serie solo se disponibili
if len(eur_usd) > 0:
...
if len(usd_sek) > 0:
    ...
    data['usd_sek'] = data['eur_sek'] / data['eur_usd']  # Calcola indirettamente

if len(dxy) > 0:
    data['dxy'] = dxy
else:
    data['dxy'] = 100  # Valore neutro

if len(vix) > 0:
    data['vix'] = vix
else:
    data['vix'] = 20  # Valore medio storico

if len(stoxx50) > 0:
    data['stoxx50'] = stoxx50
else:
    data['stoxx50'] = 4000  # Valore indicativo

if len(omxs30) > 0:
    data['omxs30'] = omxs30
else:
    data['omxs30'] = 2000  # Valore indicativo

if len(brent) > 0:
    data['brent'] = brent
else:
    data['brent'] = 80  # Valore medio

if len(copper) > 0:
    data['copper'] = copper
else:
    data['copper'] = 4  # Valore medio
...
# ============================================================================
# 2. FEATURE ENGINEERING - Multi-timeframe Technical + Macro
# ============================================================================
...
# --- Cross-correlations (Contagion effects) ---
...
# --- Interest Rate Proxies (CRITICAL for FX) ---
df['equity_spread'] = (df['stoxx50_ret_5d'] - df['omxs30_ret_5d']) * 100

# --- Carry proxy (trend following) ---
...
# --- Target: Forward returns (quello che vogliamo prevedere) ---
...
# Selezione feature (escludiamo target e prezzi raw)
...
# Target: previsione a 5 giorni (ottimale per affitto)
...
# ============================================================================
# 5. ENSEMBLE MODELING - Multiple Algorithms
# ============================================================================
...
# Ensemble: Media pesata (più peso a XGBoost)
...
# Directional Accuracy (più importante del MAE per trading decisions)
...
# ============================================================================
# 7. FORWARD FORECAST (Rolling Window) - ESTESO AD APRILE 2026
# ============================================================================
...
# Calcola numero di giorni lavorativi
...
for i in range(total_forecast_days):
    ...
    # Decay factor: mantieni features più stabili nel lungo termine
...
# Usa solo quello corto per le decisioni tattiche
...
if len(good_days) > 0:
    ...
    print(f"\n⚠️  WARNING: No days forecasted above {threshold} in next 30 days")
    print("   Recommendation: Pay early or wait for better market conditions")

# ============================================================================
# 10. VISUALIZATION - 3 GRAFICI
# ============================================================================

# GRAFICO 1: Historical + Full Forecast fino Aprile 2026
# ========================================================
...
# Forecast completo fino aprile 2026
...
# Separazione visiva tra short-term e long-term
...
# GRAFICO 2: Dettaglio Forecast Esteso (Solo Periodo Futuro)
# ===========================================================
fig2 = plt.figure(figsize=(16, 9))

# Subplot 1: Prezzo previsto
...
# Evidenzia zona short-term vs long-term
...
# Subplot 2: Costo affitto in EUR
...
# GRAFICO 3: Decisione Tattica (30 giorni - Il più importante)
# =============================================================
fig3, (ax3a, ax3b) = plt.subplots(2, 1, figsize=(14, 10))

# Panel 1: Historical recente + Forecast breve termine
...
print("   1️⃣  Full Historical + Extended Forecast (Now → April 2026)")
print("   2️⃣  Detailed Forecast Period Only (Focus on future movements)")
print("   3️⃣  Tactical Decision Chart (Next 30 days - MOST RELIABLE)")
print("\n⚠️  Remember: Charts 1-2 show extended forecast for visualization only.")
